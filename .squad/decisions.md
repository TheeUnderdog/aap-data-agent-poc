...
- **Decision:** Migrate web app from hard-coded `web/config.js` to dynamic, API-first configuration loading from `GET /api/agents` with static JSON fallback
- **Rationale:** Enables theme/agent swaps and branding changes without frontend redeployment. Configuration treated as runtime content, not bundled code.
- **Implementation:**
  - **Frontend:** `web/js/app.js` loads config from API first, falls back to `web/app.json` + `web/agents/*/agent.json`
  - **Backend:** `web/server.py` adds `GET /api/agents` endpoint returning all agent metadata
  - **Config structure:** Central `web/app.json` (branding), per-agent `web/agents/{slug}/agent.json` (metadata + routing keywords), agent load order in `web/agents/_order.json`
  - **Runtime contract:** Legacy `window.APP_CONFIG` rebuilt from API/JSON responses so existing UI code remains compatible (zero UI changes)
- **Benefits:** Reusable across customers, decoupled config from code, API-driven discovery, graceful offline fallback
- **Owner:** Linus (Frontend Dev), Basher (Backend Dev)
- **Status:** Implemented

### GET /api/agents Endpoint Specification (2026-04-29)
- **Decision:** Backend provides single endpoint serving all agent configurations with legacy field flattening for UI compatibility
- **Specification:**
  - **Route:** `GET /api/agents`
  - **Response:** JSON array of agent objects (slug, name, description, accent, textColor, icon, keywords)
  - **Legacy fields:** Also returns flattened compatibility fields (useProxy, workspaceId, fabricScopes, executiveRouting)
  - **Caching:** Agent data cached in memory; fallback to empty array on error (frontend uses static JSON)
- **Implementation notes:**
  - executiveRouting generated from each agent's routingKeywords array
  - icon normalized to `agents/{slug}/icon.svg` for every agent
- **Owner:** Basher (Backend Dev)
- **Status:** Implemented

### Per-Agent Gherkin Test Specifications (2026-04-29)
- **Decision:** Create agent-specific BDD test suites at `web/agents/{slug}/tests/agent.feature` as web app structure migrates to folder-per-agent layout
- **Rationale:** Keeps test specs beside agent config for easy regression detection. Slug-scoped `@id:` tags prevent collision if aggregated later.
- **Implementation:**
  - Six agent feature files created (crew-chief, pit-crew, gearup, ignition, partspro, diehard)
  - Shared Background: open app, wait load, switch to agent tab
  - Coverage: Welcome identity, sample questions visibility/clickability, domain-relevant chat, response validation
  - Crew Chief: Includes routing/fan-out scenario (unique agent behavior)
  - Tagging: `@agent @{slug}` feature-level, `@id:{slug}-{scenario}` per scenario
- **Integration:** Tests run with `pytest -k agent`; shared step definitions in root `tests/` reused across suites
- **Owner:** Rusty (Tester)
- **Status:** Implemented
