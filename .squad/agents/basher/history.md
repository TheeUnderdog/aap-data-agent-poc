...
- **Task:** Added `GET /api/agents` endpoint to Flask server for dynamic agent configuration discovery
- **Purpose:** Frontend now loads agent metadata from API first, with static JSON fallback for offline/local dev
- **Implementation:**
  - Route returns array of agent objects (slug, name, description, accent, textColor, icon, keywords)
  - Includes flattened legacy compatibility fields (useProxy, workspaceId, fabricScopes, executiveRouting)
  - `executiveRouting` generated from each agent's `routingKeywords` array
  - `icon` normalized to `agents/{slug}/icon.svg`
...
- **Integration:** Consumed by Linus's frontend config loader on app startup
...
### AgentClient Failed-to-Fetch Diagnosis (2026-04-29)
- **Symptom pattern:** `web/js/agent-client.js` reports `Request to "ignition" failed: Failed to fetch`, but with `useProxy: true` the browser always posts to same-origin `/api/chat`; `ignition` is just the UI agent key from `web/config.js`, not the network endpoint.
- **Most likely causes in this app:** the browser never received an HTTP response because the proxy connection dropped, the worker timed out, or backend auth/token acquisition stalled before response headers were sent.
- **Key backend behavior:** `web/server.py` calls `get_fabric_token()` before returning the SSE `Response`, so delays in `ManagedIdentityCredential`, `AzureCliCredential`, or `DeviceCodeCredential` can make `/api/chat` look hung and eventually surface as a browser fetch failure.
- **Deployment hazard:** the active Flask/container path uses workspace `82f53636-206f-4825-821b-bdaa8e089893`, but the superseded Functions path still defaults to inactive workspace `e7f4acfe-90d7-4685-864a-b5f1216fe614`; if traffic is still routed to Functions/SWA, agent IDs and workspace settings can be out of sync.
- **Evidence paths:** `web/js/agent-client.js`, `web/config.js`, `web/server.py`, `api/function_app.py`, `api/local.settings.json`, `docs/environments.md`, `tests/cua/evidence/TEST_REPORT.md`

### Config Aggregation Endpoint for Agent Metadata (2026-04-29)
- **Scope:** Added `GET /api/agents` to `web/server.py` so the frontend can fetch app config, agent ordering, and all per-agent metadata in one request.
- **Contract:** The endpoint reads `web/app.json`, `web/agents/_order.json`, and each `web/agents/{slug}/agent.json` relative to `WEB_DIR`, then returns nested config plus legacy top-level fields (`useProxy`, `workspaceId`, `fabricScopes`, `routingMode`, `llmRouting`, `executiveRouting`).
- **Backend behavior:** Each agent response forces `icon` to `agents/{slug}/icon.svg`; `executiveRouting` is derived only from non-empty `routingKeywords`; missing config files return a 404 with the missing relative path for easier setup/debugging.
- **Validation:** Confirmed the route compiles, returns the expected merged payload against the current `web/app.json` + `web/agents/*` config set, and also returns the expected merged payload when pointed at a scratch config tree.
